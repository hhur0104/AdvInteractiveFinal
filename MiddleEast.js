class MiddleEast {
    ...
    constructor(state) {
        ...
        state.pergdp_me.map( d=> {
            this.perGDPmap[d.Country] = [
                d.X2004, d.X2005, d.X2006, 
        ...
        this.asia2_size = this.perGDPmap["Iran"].length 
        this.asia2_year_list = d3.range(0, this.asia2_size).map( d => d + 2004);
        ...
        this.sliderTime = d3.sliderBottom()
            .min(0)
            .max(this.asia2_size - 1)
            .step(1)
            .width(state.width * 3/4 - 20)
            .tickFormat(i => dataTime[i])
            .ticks(this.asia2_size)
            .handle(d3.symbol().type(d3.symbolCircle))
            .fill("#69b3a2")
            .on('onchange', val => {
                console.log(val)
                d3.select("#tipDiv-asia2").selectAll('circle').attr("r",0);

                clearInterval(state.asia2_timer)
                
                state.playing_a2 = false;
                state.asia2_i = val;
                state.asia2.changeColor(state);  
                
                d3.select('#year_asia2').text(this.asia2_year_list[state.asia2_i]);  
                    
                d3.selectAll(".cir_asia2_"+this.asia2_year_list[state.asia2_i])
                        .transition()
                        .duration(300)
                        .attr("r",7);
            });

        var gTime = d3.select('div#slider-asia2')
            .append('svg')
        ...
        var Tooltip = d3.select("#asia2")
            .append("div")
            .style("opacity", 0)
            .attr("id", "tipDiv-asia2")
            
        
        this.svg = d3.select("#asia2")
            .append("svg")
        ...
        this.svg.append("text")
            ...
            .attr("id","year_asia2")
            .text(this.asia2_year_list[state.asia2_i]);
        ...
        function reset(event) {
            ...
            var tipDiv = d3.select("#tipDiv-asia2")
            ...
            var svg = d3.select("#asia2").select("SVG");
        ...
        function clicked(event, d, gdpmap) {
            ...
            // );
            console.log("clicked, num_eur_sel: ", state.num_asia2_selected)


            var tipDiv = d3.select("#tipDiv-asia2")
            if (state.num_asia2_selected == 3) {
                state.num_asia2_selected = 1;
                tipDiv.selectAll("g").remove();
                tipDiv.selectAll("svg").remove();
                tipDiv.selectAll("h2").remove();
                tipDiv.selectAll("path").remove();
            ...
            tipDiv.attr("width", 300)
                .attr("height", 200)
                .style("top", state.drag_y_asia2 + "px")
                .style("left", state.drag_x_asia2 + "px")
            ...
            var dragHandler = d3.drag()
                .on("drag", function (d) {
                    console.log("asia2 tip d:", d)
                    //state.drag_y_asia2 = d.y+200
                    state.drag_y_asia2 = d.sourceEvent.pageY
                    state.drag_x_asia2 = d.x+100
                    console.log("state.drag_y_asia2", state.drag_y_asia2)
                    console.log("state.drag_x_asia2", state.drag_x_asia2)
                    d3.select(this)
                        .style("top", state.drag_y_asia2  + "px")
                        .style("left", state.drag_x_asia2 + "px");
                });
    ...
    changeColor(state) {
        this.svg.selectAll("path.countries")
            ...
            .attr("fill",d => { 
                if(this.perGDPmap[d.properties.admin] != undefined) {
                    if (this.perGDPmap[d.properties.admin][state.asia2_i] =='NA') {
                        return "LightGray";
                    } else {
                        return this.color(this.perGDPmap[d.properties.admin][state.asia2_i]);
                    }
                } else {
                    return "LightGray";
                }
            })
        this.sliderTime.silentValue(state.asia2_i)
    }

    animate(state) {
        var timer;  // create timer object
        var year_list = this.asia2_year_list
        var asia2_max = this.asia2_size
        d3.select('#play-asia2')  
            .on('click', function() { 
                
                if(state.playing_a2 == false) {  
                    timer = setInterval(function() {
                    d3.select("#tipDiv-asia2").selectAll('circle').attr("r",0);
                    
                    if(state.asia2_i < asia2_max) {  
                        state.asia2_i +=1;  
                    } else {
                        state.asia2_i = 0;  
                    }
                    ...
                    d3.select('#year_asia2').text(year_list[state.asia2_i]);  
                    
                    d3.selectAll(".cir_asia2_"+year_list[state.asia2_i])
                        .transition()
                        .duration(300)
                        .attr("r",7);

                }, 1000);
                state.asia2_timer = timer
                state.playing_a2 = true;   
                } 
        });

        d3.select('#stop-asia2')  
          .on('click', function() {  
            if(state.playing_a2 == true) {  
                clearInterval(state.asia2_timer);   
                state.playing_a2 = false;
            } 
        });
